...
import { DashboardIcon, PackageIcon, StoreIcon, SettingsIcon, LogoutIcon, AderaLogo, ChevronLeftIcon, ChevronRightIcon, FinancialsIcon } from './Icons';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setView, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      <div className={`fixed inset-y-0 left-0 z-30 bg-gray-800 border-r border-gray-700 transform transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-20'} hidden md:flex flex-col`}>
        <div className="flex items-center justify-between h-20 px-6 border-b border-gray-700">
           <div className={`transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                <AderaLogo className="h-9 w-auto text-teal-400" />
           </div>
          <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-full text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none">
        ...
        <div className="px-4 py-6 border-t border-gray-700">
            ...
            >
             ...
             <button
              className={`flex items-center w-full px-4 py-3 mt-2 rounded-lg transition-colors duration-200 text-gray-400 hover:bg-gray-700 hover:text-white ${!isOpen && 'justify-center'}`}
            >
       ...
       {/* Mobile Sidebar */}
        ...
        <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-gray-800 border-r border-gray-700 transform transition-transform duration-300 ease-in-out md:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
           <div className="flex items-center justify-between h-20 px-6 border-b border-gray-700">
             <AderaLogo className="h-9 w-auto text-teal-400" />
           </div>
           <nav className="flex-1 px-4 py-6 space-y-2">
